//! Verbatim transcription of `src/crypto/ec2b_tables.hpp`.
//!
//! These are the standard AES inverse S-box, inverse ShiftRows permutation and
//! GF(2^8) multiply-by-9/11/13/14 tables, plus the fixed `roundKeys` and
//! `keyXORPadTable` constants that the EC2B stream scrambler bakes in. Keep them
//! bit-identical to the C++ header — `unscramble_aes_key` is only correct if
//! every entry matches.

pub const KEY_XOR_PAD_TABLE: [u8; 16] = [
...
pub const SHIFT_ROWS_INV: [u8; 16] = [
